...
app.post("/courses/add", upload.single("image"), (req, res) => {
  ...
  const q = "INSERT INTO courses (name, level, description, image) VALUES (?,?,?,?)";
...
app.post("/add-batch", (req, res) => {
  ...
  if (!course_id || !batch_name || !start_time || !end_time) return res.status(400).json({ message: "All fields required" });

  const q = "INSERT INTO batches (course_id, batch_name, start_time, end_time) VALUES (?,?,?,?)";
...
app.post("/schedule-lecture", (req, res) => {
  ...
  if (!batch_id || !instructor_id || !lecture_date) return res.status(400).json({ message: "All fields required" });

  const q = "INSERT INTO scheduled_lectures (batch_id, instructor_id, lecture_date) VALUES (?,?,?)";
  db.query(q, [batch_id, instructor_id, lecture_date], (err, result) => {
    if (err) {
      if (err.code === "ER_DUP_ENTRY") return res.status(400).json({ message: "Instructor already has a lecture on this date" });
...
app.post("/login", (req, res) => {
  ...
  const query = "SELECT id, name FROM instructors WHERE email = ? AND password = ?";
